exports.handler = async function(event) {
  ...
  const mlbFetch = (url) => fetch(url, { headers: { accept: "application/json", "user-agent": "MLB-Live-Command-Center/11.14" } });
  ...
  try {
    ...
    if (q.type === "rosters") {
      ...
      const entries = await Promise.all(teamIds.map(async teamId => {
        const url = `https://statsapi.mlb.com/api/v1/teams/${teamId}/roster?rosterType=active&date=${encodeURIComponent(date)}&hydrate=person`;
        const response = await mlbFetch(url);
        ...
        return [teamId, {
          roster: (data.roster || []).map(entry => ({
            ...
            pitchHand: entry.person?.pitchHand?.code || null
    ...
    // Review-only context for the validated reliever-choice model. This endpoint is
    // deliberately NOT called on every pitch. It reconstructs the exact pregame 30-day
    // workload/role features from completed MLB games strictly before the requested date.
    if (q.type === "bullpenContext") {
      ...
      const schedResp = await mlbFetch(schedUrl);
      ...
      for (const d of (sched.dates || [])) {
        for (const g of (d.games || [])) {
          const st = g.status || {};
          if (st.abstractGameState === "Final" || ["Final","Game Over","Completed Early"].includes(st.detailedState)) {
            games.push({ gamePk: g.gamePk, date: d.date });
          }
        }
      }
      const feeds = await Promise.all(games.map(async g => {
        try {
          const r = await mlbFetch(`https://statsapi.mlb.com/api/v1.1/game/${g.gamePk}/feed/live`);
      ...
      for (const item of feeds.filter(Boolean)) {
        ...
        for (const play of plays) {
          ...
          x.bf += 1;
          x.firstInning = Math.min(x.firstInning, inning);
          perPitcher.set(pid, x);
        }
        ...
        for (const [pid, x] of perPitcher.entries()) {
          ...
          z.apps += 1; z.bf += x.bf;
          z.lateApps += x.firstInning >= 7 ? 1 : 0;
          z.saveLikeApps += x.firstInning >= 9 ? 1 : 0;
          z.lastDayOrd = z.lastDayOrd == null ? dayOrd : Math.max(z.lastDayOrd, dayOrd);
          hist.set(pid, z);
        }
      }
      ...
      const rosterResp = await mlbFetch(rosterUrl);
