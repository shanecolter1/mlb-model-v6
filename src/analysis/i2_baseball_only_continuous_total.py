...
def main():
    ...
    v=pd.read_csv(next(Path(a.v04).rglob('v04_oos_predictions.csv'))); m=v.merge(d[d.game_id.notna()].drop_duplicates('game_id')[['game_id','baseball_expected_total']],on='game_id',how='left',validate='one_to_one'); jr=float(m.baseball_expected_total.notna().mean()); print('join_rate',jr); m=m[m.baseball_expected_total.notna()].copy(); print('common_n',len(m),'of',len(v))
