...
def find_score_cols(df):
    pairs=[('away_score','home_score'),('away_runs','home_runs'),('away_team_runs','home_team_runs'),('away_final_score','home_final_score'),('visitor_score','home_score')]
    for a,h in pairs:
...
def build_continuous_total(df, window_days=365, shrink_games=30.0):
    ...
    # Maintain prior-date team offense and defense game logs. Same-day games never enter each other's features.
    hist_off={}; hist_def={}; league=[]
    out=[]
    for date,grp in d.groupby('game_date', sort=True):
        ...
        for store in [hist_off,hist_def]:
            for t in list(store):
                store[t]=[(dt,r) for dt,r in store[t] if dt>=cutoff]
        ...
        for idx,r in grp.iterrows():
            ...
            def shrunk(store,t):
                vals=[v for _,v in store.get(t,[])]
                n=len(vals); s=sum(vals)
                return (s+shrink_games*league_mean)/(n+shrink_games), n
            a_off,nao=shrunk(hist_off,a); h_off,nho=shrunk(hist_off,h)
            a_def,nad=shrunk(hist_def,a); h_def,nhd=shrunk(hist_def,h)
            away_exp=0.5*(a_off+h_def)
            home_exp=0.5*(h_off+a_def)
            out.append((idx,away_exp+home_exp,away_exp,home_exp,min(nao,nho,nad,nhd)))
        # update only after all games on the date are scored
        for idx,r in grp.iterrows():
            a=norm_code(r.away_team_code); h=norm_code(r.home_team_code)
            ar=float(r.away_score_num); hr=float(r.home_score_num)
...
def fit_logistic_ridge(X,y,lam=1.0,max_iter=100):
    ...
    for _ in range(max_iter):
        eta=X@beta; p=1/(1+np.exp(-np.clip(eta,-40,40)))
...
def main():
    ...
    master=next(Path(a.phase1).rglob('MLB_Game_Stats_Joined_2021_2025.csv.gz'))
    ...
    sca,sch=find_score_cols(d); d['away_score_num']=pd.to_numeric(d[sca],errors='coerce'); d['home_score_num']=pd.to_numeric(d[sch],errors='coerce')
    d['i2_runs']=pd.to_numeric(d['inning2_total_runs'],errors='coerce'); d['actual_under']=(d.i2_runs==0).astype(int)
    d=d.dropna(subset=['game_date','away_score_num','home_score_num','i2_runs']).copy()
    d=build_continuous_total(d)
    # Join current v0.4 OOS predictions using season/date/teams/game number, robust to lack of game_id in canonical master.
    ...
    # Prefer exact retro ID if common; else season/date/team/game_number.
    if 'retro_game_id' in d.columns and 'retro_game_id' in v.columns:
        m=v.merge(d,on='retro_game_id',how='left',suffixes=('','_hist'))
    else:
    ...
    # Rolling annual OOS tuning. Candidate spline complexity and ridge chosen only from prior seasons.
    candidates=[]
    for nknots in [0,2,3,4]:
        for lam in [0.1,1.0,10.0,100.0]: candidates.append((nknots,lam))
    pred_rows=[]; metric=[]
    for season in [2022,2023,2024,2025]:
        tr=m[m.season<season].copy(); te=m[m.season==season].copy()
        if tr.empty or te.empty: continue
        # tune using leave-last-season-forward validation within available prior seasons when possible; else simple in-sample penalized choice favors lower complexity
        prior_seasons=sorted(tr.season.unique())
        scores=[]
        for nknots,lam in candidates:
            ...
            if len(prior_seasons)>=2:
                for vs in prior_seasons[1:]:
                    ...
                    qs=np.linspace(.2,.8,nknots+2)[1:-1] if nknots else []
                    knots=np.quantile(tt.baseball_expected_total,qs) if nknots else []
                    X= spline_design(tt.baseball_expected_total,knots); b=fit_logistic_ridge(X,tt.y.to_numpy(),lam)
                    fs.append(logloss(vv.y,predict(b,spline_design(vv.baseball_expected_total,knots))))
            else: fs=[999+nknots*1e-3]
            scores.append((np.mean(fs),nknots,lam))
        _,nk,lam=min(scores)
        qs=np.linspace(.2,.8,nk+2)[1:-1] if nk else []; knots=np.quantile(tr.baseball_expected_total,qs) if nk else []
        beta=fit_logistic_ridge(spline_design(tr.baseball_expected_total,knots),tr.y.to_numpy(),lam)
        p=predict(beta,spline_design(te.baseball_expected_total,knots))
    ...
    oos=pd.concat(pred_rows,ignore_index=True)
    # Compare to v0.4 on common OOS rows. p_under_local_cv should exist.
    comps=[]
    ...
    keep=[c for c in ['season','game_date','away_team_code','home_team_code','baseball_expected_total','p_under_baseball_continuous','p_under_local_cv','actual_under','selected_knots','selected_lambda'] if c in oos.columns]
    oos[keep].to_csv(out/'oos_predictions.csv',index=False)
    # Shape diagnostics
    def moments(s):
