...
def core_matrix(df,event,window):
    b=f'batter_{window}_{event}_rate'; p=f'pitcher_{window}_{event}_rate'; bv=df[b].to_numpy(float); pv=df[p].to_numpy(float); inn=df.inning.to_numpy(int)
    cols=[np.ones(len(df))]+[(inn==i).astype(float) for i in range(2,10)]+[bv,pv,bv*pv]
...
def evaluate(x,event,family,window,year,ridge):
    ycol='y_'+event; ratecols=[f'batter_{window}_{event}_rate',f'pitcher_{window}_{event}_rate']; fcols=family_cols(family,window); needed=['season','inning','batter_side','pitcher_hand',ycol]+ratecols+fcols
    tr=x[x.season<year][needed].dropna().copy(); te=x[x.season==year][needed].dropna().copy()
    if len(tr)<5000 or len(te)<1000: return None
    ytr=tr[ycol].to_numpy(float); yte=te[ycol].to_numpy(float); Ctr=core_matrix(tr,event,window); Cte=core_matrix(te,event,window); Ftr,Fte=standardize(tr,te,fcols)
