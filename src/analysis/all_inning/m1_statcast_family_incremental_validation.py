...
Selection: 2022 and 2023 only. Confirmation: 2024 once per selected event/family
spec. 2025 is never loaded. Each challenger and its core comparator are scored
on identical complete-case rows. No missing-value imputation is introduced.
"""
...
FAMILY_TEMPLATES={
 'discipline_contact':[
   'batter_{w}_swing_rate','batter_{w}_whiff_rate','batter_{w}_chase_rate','batter_{w}_contact_rate','batter_{w}_zone_rate',
   'pitcher_{w}_swing_rate','pitcher_{w}_whiff_rate','pitcher_{w}_chase_rate','pitcher_{w}_contact_rate','pitcher_{w}_zone_rate'],
 'pitch_mix_matchup':[
   'pitcher_{w}_mix_fb','pitcher_{w}_mix_breaking','pitcher_{w}_mix_offspeed',
   'batter_{w}_fb_whiff_rate','batter_{w}_breaking_whiff_rate','batter_{w}_offspeed_whiff_rate',
   'batter_{w}_fb_xwoba_contact','batter_{w}_breaking_xwoba_contact','batter_{w}_offspeed_xwoba_contact'],
 'velocity_spin_extension':[
   'pitcher_{w}_fb_velo','pitcher_{w}_fb_spin','pitcher_{w}_extension',
   'batter_{w}_fb_velo','batter_{w}_fb_spin','batter_{w}_extension'],
 'contact_quality_expected':[
   'batter_{w}_avg_ev','batter_{w}_hardhit_rate','batter_{w}_barrel_rate','batter_{w}_avg_launch_angle','batter_{w}_xwoba_contact','batter_{w}_xba_contact',
   'pitcher_{w}_avg_ev','pitcher_{w}_hardhit_rate','pitcher_{w}_barrel_rate','pitcher_{w}_avg_launch_angle','pitcher_{w}_xwoba_contact','pitcher_{w}_xba_contact']
}
...
def core_matrix(df,event,window):
    b=f'batter_{window}_{event}_rate'; p=f'pitcher_{window}_{event}_rate'
    bv=df[b].to_numpy(float); pv=df[p].to_numpy(float); inn=df.inning.to_numpy(int)
    cols=[np.ones(len(df))]
    for i in range(2,10): cols.append((inn==i).astype(float))
    cols += [bv,pv,bv*pv]
    if event in PLATOON_EVENTS:
        bl=(df.batter_side.astype(str).str.upper()=='L').astype(float).to_numpy(); pl=(df.pitcher_hand.astype(str).str.upper()=='L').astype(float).to_numpy()
        cols += [bl,pl,bl*pl]
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--m1-matrix',type=Path,required=True); ap.add_argument('--batter-asof',type=Path,required=True); ap.add_argument('--pitcher-asof',type=Path,required=True); ap.add_argument('--output-dir',type=Path,required=True)
    ...
    for r in selected.itertuples():
        q=evaluate(x,r.event,r.family,r.window,2024,float(r.ridge))
        if q is not None: confirmations.append(q)
    conf=pd.DataFrame(confirmations).merge(selected,on=['event','family','window','ridge'],how='left',validate='one_to_one')
    conf['development_logloss_positive_all_years']=conf.dev_worst_logloss_improvement>0
    conf['confirmation_logloss_positive']=conf.logloss_improvement>0
    conf['confirmation_brier_positive']=conf.brier_improvement>0
    conf['earns_incremental_candidate_status']=conf.development_logloss_positive_all_years & conf.confirmation_logloss_positive & conf.confirmation_brier_positive
    folds.to_csv(a.output_dir/'m1_statcast_family_development_folds.csv',index=False); s.to_csv(a.output_dir/'m1_statcast_family_development_summary.csv',index=False); selected.to_csv(a.output_dir/'m1_statcast_family_selected_specs.csv',index=False); conf.to_csv(a.output_dir/'m1_statcast_family_confirmation_2024.csv',index=False)
