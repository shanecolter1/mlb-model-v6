...
def main():
    ...
    required_outcomes = ["game_id", "inning", "away_runs", "home_runs", "away_half_played", "home_half_played"]
    missing = [c for c in required_outcomes if c not in outcomes.columns]
    if missing:
        raise RuntimeError(f"inning_outcomes missing {missing}")
    outcomes = outcomes[required_outcomes].copy()
    ...
    for half in ("top", "bottom"):
        ...
        if half == "top":
            ...
            r["runs_half"] = pd.to_numeric(r["away_runs"], errors="coerce")
            ...
            r["runs_half"] = pd.to_numeric(r["home_runs"], errors="coerce")
            r["half_played"] = r["home_half_played"].fillna(False).astype(bool)
        r["half"] = half
        r["scored_half"] = np.where(r["half_played"], (r["runs_half"].fillna(0) >= 1).astype(int), np.nan)
