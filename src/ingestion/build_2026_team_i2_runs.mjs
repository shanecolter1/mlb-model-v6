...
async function j(u){const r=await fetch(u,{headers:{accept:'application/json','user-agent':'MLB-I2-TeamRates/1.0'}});if(!r.ok)throw new Error(`${r.status} ${u}`);return r.json()}
...
for(const [idx,g] of games.entries()){
 ...
 if(idx%100===0) console.log(idx,'/',games.length);
}
