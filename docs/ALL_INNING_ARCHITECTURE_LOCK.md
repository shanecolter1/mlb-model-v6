...
- The PA-history/reliability gate rebuilt strictly prior-date player counts that reproduce the locked 365-day core rates to floating-point precision, then tested candidate windows, league-prior shrinkage strengths, cross-event profiles, event composition, and sample-reliability curves against the retained core + platoon + Statcast comparator. All sixteen marginal event-family candidates were positive on the prescribed 2022 selection, 2023 validation, and 2024 development confirmation rule. The joint conditional gate retained sample reliability for K; cross-event profile + sample reliability for baserunner; cross-event profile + target-history shrinkage for HR; and cross-event profile + sample reliability for non-HR hit. Their 2024 log-loss improvements over the full retained comparator were 0.000585, 0.000701, 0.000119, and 0.000190 respectively, with positive Brier improvements. These are development-selected M1 inputs only; they do not promote M4 or open 2025.
...
8. Systematically screen additional pitcher/batter statistic families for incremental PA-event predictive value on 2021-2024 only; integrate only validated additions into both P and D branches. IN PROGRESS: PLATOON, INITIAL STATCAST FAMILY/REDUNDANCY, AND PA-HISTORY/RELIABILITY-SHRINKAGE GATES COMPLETE; AVAILABLE STATCAST EXTENSIONS MUST STILL BE TESTED BEFORE FEATURE FREEZE.
