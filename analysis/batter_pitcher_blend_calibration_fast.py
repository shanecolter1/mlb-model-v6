...
Newton-Cholesky solves the same L2 multinomial objective efficiently for n >> p.
"""
...
OUT=core.OUT;MAX_ITER=120

def fit_newton(X,y,C):
    m=core.LogisticRegression(C=C,solver='newton-cholesky',max_iter=MAX_ITER,tol=1e-7)
    m.fit(X,y)
    if int(np.max(m.n_iter_))>=MAX_ITER:raise RuntimeError(f'Newton-Cholesky convergence gate failed at {MAX_ITER} iterations')
    return m
core.fit=fit_newton

def main():
    OUT.mkdir(parents=True,exist_ok=True);plays,prov=core.fetch_plays()
    ...
    for h in core.HALF_LIVES:
        Xs,y,yrs,_=core.build_multi_prior(plays,h,core.PRIORS);tr=np.isin(yrs,[2021,2022,2023]);sel=yrs==2024
        for prior in core.PRIORS:
            X=Xs[prior];m=core.fit(X[tr],y[tr],fixed_C);met=core.metrics(y[sel],m.predict_proba(X[sel]));row={'stage':'decay_prior','half_life_days':h,'prior_strength':prior,'C':fixed_C,'logloss_2024':met['logloss'],'brier_2024':met['brier'],'max_n_iter':int(np.max(m.n_iter_))};stage1.append(row)
            if best is None or (met['logloss'],met['brier'])<(best[0],best[1]):best=(met['logloss'],met['brier'],h,prior)
        del Xs,y,yrs;gc.collect()
    _,_,h,prior=best
    Xs,y,yrs,_=core.build_multi_prior(plays,h,[prior]);X=Xs[prior];tr=np.isin(yrs,[2021,2022,2023]);sel=yrs==2024;stage2=[];bestC=None
    for C in core.CS:
        m=core.fit(X[tr],y[tr],C);met=core.metrics(y[sel],m.predict_proba(X[sel]));row={'stage':'regularization','half_life_days':h,'prior_strength':prior,'C':C,'logloss_2024':met['logloss'],'brier_2024':met['brier'],'max_n_iter':int(np.max(m.n_iter_))};stage2.append(row)
        if bestC is None or (met['logloss'],met['brier'])<(bestC[0],bestC[1]):bestC=(met['logloss'],met['brier'],C)
    C=bestC[2];del Xs,y,yrs;gc.collect()
    ...
    convergence={'solver':'newton-cholesky','max_iter':MAX_ITER,'final_fit_n_iter':int(np.max(m.n_iter_)),'converged':int(np.max(m.n_iter_))<MAX_ITER}
    ...
    params={'version':'joint-multinomial-pa-v1','classes':core.CLASSES,'feature_order':[f'batter_log_ratio_{e}' for e in core.NONOUT]+[f'pitcher_log_ratio_{e}' for e in core.NONOUT]+[f'league_logit_{e}_vs_out' for e in core.NONOUT],'selected_hyperparameters':manifest['selected_hyperparameters'],'coef':m.coef_.tolist(),'intercept':m.intercept_.tolist(),'sklearn_classes':m.classes_.tolist(),'convergence':convergence,'validation_2025':validation,'production_eligible':False}
    (OUT/'model_development_manifest.json').write_text(json.dumps(manifest,indent=2));(OUT/'joint_multinomial_pa_model.json').write_text(json.dumps(params,indent=2));(OUT/'end_2024_skill_state.json').write_text(json.dumps(core.build_state_through_2024(plays,h),separators=(',',':')))
    rows=stage1+stage2
    with (OUT/'multinomial_grid_2024.csv').open('w',newline='') as f:
        w=csv.DictWriter(f,fieldnames=['stage','half_life_days','prior_strength','C','logloss_2024','brier_2024','max_n_iter']);w.writeheader();w.writerows(rows)
